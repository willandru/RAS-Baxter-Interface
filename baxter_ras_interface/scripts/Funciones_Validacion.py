...
def callback(data):
    ...
    Validacion_script.main(matriz,dificultad,avanzar)
    terminar='Acabe'
    rospy.loginfo(terminar)
    pub.publish(terminar)   
def main():
    global pub
    rospy.init_node('solucion_validacion')
    pub = rospy.Publisher('Terminamos', String, queue_size=10)
    rospy.Subscriber("/Validacion_script", Int32MultiArray, callback)
    rospy.spin()
